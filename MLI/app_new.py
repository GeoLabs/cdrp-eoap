...
def main(model_file,dem_path,hs_path,slope_path,pre_image_path,post_image_path,mask_path,boundary_path):
    ...
    catalog = pystac.Catalog(id='result-catalog', description='This catalog contains the results of the processing.')
    bbox, footprint = get_bbox_and_footprint(rasterPath)
    datetime_utc = datetime.now(tz=timezone.utc)
    item = pystac.Item(id='myMockStacItem',
                    geometry=footprint,
                    bbox=bbox,
    ...
    print(json.dumps(catalog.to_dict(), indent=4))
    print(json.dumps(item.to_dict(), indent=4))
    open(OUTPUT_PATH+"/myMockStacItem.json","w").write(json.dumps(item.to_dict(), indent=4))
